...
PROJECT_DIR_ON_REMOTE_MACHINE="/home/duroakos/mainframer/$PROJECT_DIR_NAME"
...
function syncAfterRemoteCommand {
    ...
    COMMAND="rsync --archive --ignore-errors --delete --compress-level=$REMOTE_COMPRESS_LEVEL "
    #COMMAND="rsync -rltzuv --delete --compress-level=$REMOTE_COMPRESS_LEVEL "
    ...
    endTime="$(date +%s)"
    echo "Sync done: took $(formatTime $((elndTime-startTime)))."
}
